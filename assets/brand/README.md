...
Every asset in this directory is **generated**, and the generator lives in the sibling `enterprise-security-homelab` repository. Do not edit the SVGs by hand here. Change the source there, rebuild, and copy the output across:

```bash
python scripts/brand/build_brand.py   # in enterprise-security-homelab
```
